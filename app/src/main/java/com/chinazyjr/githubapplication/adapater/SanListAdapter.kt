...
class SanListAdapter(var list: List<BorrowPlanListBean.Model.X>) : BaseQuickAdapter<BorrowPlanListBean.Model.X, BaseViewHolder>(R.layout.item_product_list, list) {
    override fun convert(helper: BaseViewHolder?, item: BorrowPlanListBean.Model.X) {
        if(helper?.layoutPosition==0){
            helper?.setVisible(R.id.ll_title,true)
            helper?.setText(R.id.tv_group,"散标")
        }else{
